...
namespace Unity.Services.Samples.Parties
{
    public class LobbyListView : MonoBehaviour
    {
        ...
        List<LobbyEntryView> m_LobbyEntryViews = new List<LobbyEntryView>();

        public void Init(int maxLobbySize)
        {
            for (int i = 0; i < maxLobbySize; i++)
            {
                var entry = Instantiate(m_LobbyEntryPrefab, m_ContentLayoutGroup.transform);
                m_LobbyEntryViews.Add(entry);
                entry.Init();
            }
        ...
        IEnumerator GrowScrollListToContent()
        {
            ...
            foreach (var entry in m_LobbyEntryViews)
            {
        ...
        {
            SetAllEmpty();
            var localPlayerEntry = m_LobbyEntryViews.First();

            //Copy the view list without the player
            var nonLocalPlayerViews = new List<LobbyEntryView>(m_LobbyEntryViews);
